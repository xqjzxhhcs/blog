...
router.use(function(req,res,next){
    ...
    if(req.session.userInfo==null || req.session.userInfo==undefined || req.session.userInfo.isAdmin==0){
        res.send("<script>alert('非法操作，请先登录');location.href='http://127.0.0.1:8888/'</script>");
        return;
    }
    next();
});
    ...
    pool.getConnection(function(err,conn){
        conn.query("select * from contents where tid=?",[tid],function(err,resu){
            if(err){
                ...
                //意味着这个tid在contents里面有数据，所以不能删除
                //这个时候，要想删除，那么，就要把contents里面所有的内容为tid的数据，转换为首页的数据
...
router.get("/content",function(req,res){
    ...
    pool.getConnection(function(err,conn){
        conn.query("select * from contents",function(err,result){
            ...
            //开始分页查询
            conn.query("select c.*,t.tname,u.uname from contents c,type t,user u" +
                " where c.tid=t.tid and c.uid=u.uid order by cid limit ?,?",[beginSize,size],function(err,result2){
                //释放链接
                conn.release();
                res.render("admin/content_index",{
                    userInfo:req.session.userInfo,
                    contents:result2,//与网页中的{% for c in contents %}contents 匹配，随便写一个abc,不写type/user/contents也行
                    tag:'content',//与router.get("/content",function(req,res)里面的content对应
                    page:page,
                    pages:pages,
                    size:size,
                    count:count,
...
router.get("/content/edit",function(req,res){
    ...
    pool.getConnection(function(err,conn){
        conn.query("select * from type order by tid",function(err,resu){
            conn.query("select c.*,t.tname from contents c,type t " +
                "where c.tid=t.tid and c.cid=?",[cid],function(err,result){
                conn.release();
                if(err){
                    console.log(err);
                }
                res.render("admin/content_edit",{
                    userInfo:req.session.userInfo,
                    content:result[0],
                    types:resu  //将查询到的resu(type的种类存到types里面)
                })
            });
        })
...
router.post("/content/add",function(req,res){
    ...
    pool.getConnection(function(err,conn){
        conn.query("insert into contents values(0,?,?,?,?,?,?,0,0,0)",
